...
/* Check for symbols. */
		...
		{
/* ... */
			if(l_i + strlen(pg_symbols[l_j]) <= *pt_input_source_buffer_size && strncmp(pg_symbols[l_j], pt_input_source_buffer + l_i, strlen(pg_symbols[l_j])) == 0)
			{
/* Resize the buffer of scanned tokens. */
				(*pt_output_token_buffer_size)++;
				*ppt_output_token_buffer = realloc(*ppt_output_token_buffer, *pt_output_token_buffer_size * sizeof(token_t));
...
/* Move to the closing quote. */
			...
			if(pt_input_source_buffer[l_i] != '\'')
			{
/* Unterminated character literal, terminate the scanning process. */
...
/* Append the character to the string buffer. */
				pl_string_buffer = realloc(pl_string_buffer, l_string_length + 1);
				pl_string_buffer[l_string_length++] = l_character;
...
/* Add the plaintext to the buffer. */
			(*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].pt_plaintext_buffer = pl_string_buffer;
			(*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_plaintext_buffer_size = l_string_length;

/* Add the token to the buffer. */
			(*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_value.pt_string8_literal = pl_string_buffer;
...
/*
 *	Keep adding characters to the identifier's name until a	non-identifier
...
/* Set the vague token type. */
			if(l_is_floating_point) (*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_token_type = (uint8_t)TOKEN_TYPE_FLOAT32_LITERAL;
			else if(!l_is_floating_point) (*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_token_type = (uint8_t)TOKEN_TYPE_INT32_LITERAL;

/*
 *	Narrow down the token type based on the suffix.
 *	Also make sure the default type is set, that being a 32-bit integer
 *	literal.
 */
			uintmax_t l_j = 0;
...
/* ... */
				...
				{
/*
 *	We're dealing with a long long literal, which is guaranteed to be 64-bit.
 *	We must also preserve it's signedness, just like the long literal's
 *	handling above has done so.
 */
...
/* ... */
				...
				{
/*
 *	It's a long literal, however we're not sure yet if it's a 32-bit or 64-bit
 *	integer, and therefore, we must check the size of the long type.
 *	We must also preserve it's signedness, instead of overriding it.
 */
					uint8_t l_existing_signedness = (*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_token_type & 0b1;

/* ... */
					if(sizeof(long) == 8) (*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_token_type = (TOKEN_PREFIX_INTEGER_LITERAL << 1) | (l_existing_signedness << 3) | (TOKEN_SUBTYPE_INT64 << 4);
					else if(sizeof(long) == 4) (*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_token_type = (TOKEN_PREFIX_INTEGER_LITERAL << 1) | (l_existing_signedness << 3) | (TOKEN_SUBTYPE_INT32 << 4);
...
/*
 *	If there's no suffix, but a floating point number was detected, then simply
 *	submit the token as a floating point literal, but without a known size.
 *	If there's no suffix, but an integer was detected, then simply submit the
 *	token as a 32-bit integer literal.
 */
			if(l_suffix_length == 0 && l_is_floating_point) (*ppt_output_token_buffer)[(*pt_output_token_buffer_size) - 1].t_token_type = (uint8_t)TOKEN_TYPE_FLOAT64_LITERAL;
